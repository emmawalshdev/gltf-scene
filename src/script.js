...
.onChange(updateAllMaterials); // update value on change
...
// directionalLight.shadow.normalBias = 0.05; // prevent casting shadow on itself
...
gui
  .add(directionalLight.position, 'x').min(-5).max(5).step(0.001).name('lightX');
gui
  .add(directionalLight.position, 'y').min(-5).max(5).step(0.001).name('lightY');
gui
  .add(directionalLight.position, 'z').min(-5).max(5).step(0.001).name('lightZ');
/**
 * Sizes
 */
const sizes = {
...
const renderer = new THREE.WebGLRenderer({
    canvas: canvas,
    antialias: true, // has to be on instantiation
})
renderer.setSize(sizes.width, sizes.height)
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
